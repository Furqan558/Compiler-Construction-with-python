...
lookAhead = 0
while lookAhead < len(inputBuffer):
    row = lst[0].index(stack[-1])
    col = lst[0].index(inputBuffer[lookAhead])
    if inputBuffer[lookAhead] == "$" and stack[0] == "$":
        print("Accepted")
        break
    if lst[row][col] == ">":
        print(lst[0][col] ,end=' ')
        ...
        print(lst[row][0] ,end=' ')
        print(lst[row][col])
        stack.append(inputBuffer[lookAhead])
        lookAhead+=1
    else:
        print("Not Accepted")
        break
